...
#start paths on circle of radius r
#step size h2 for euler's method to generate paths

f(x,y) = 2.0 + sin(x+pi/2) + sin(y+pi/2) #the function
grad(x,y) = [-sin(x), -sin(y)] #the gradient
...
gpx=Float64[]
gpy=Float64[]
start=(cos(h*(i-1.1))*r,sin(h*(i-1.1))*r)
push!(gpx,start[1])
push!(gpy,start[2])
...
#calculate end point of paths on boundary
 if gpx[end]>x_max
...
##########################################################
function curv(r)
#calculate curvature of isosurface at point r
#taken from Tim's code

 f(x,y) = 2.0 + sin(x+pi/2) + sin(y+pi/2) #the function
 grad(x,y) = [-sin(x), -sin(y)] #the gradient
 hess(x,y) = [-cos(x) 0; 0 -cos(y)] #the hessian
...
#calculate area of square using gp-only integration
#start gradient paths on circle of radius intradius
#use step size h for generating paths and integration
...
#allocate array of vectors for l1(s)
...
area=0.0 #integrate trapezoidal rule
for i=1:n
n2=length(arcs[i])
 for k=1:n2-1
  x1=p[2i-1][k]
...
@printf "\n Use nonradial function 2.0+sin(x+pi/2)+sin(y+pi/2) for method"
